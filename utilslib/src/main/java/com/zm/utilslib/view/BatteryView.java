...
public class BatteryView extends View {
    ...
    private void drawHorizontalBattery(Canvas canvas) {
        ...
        RectF r2 = new RectF(strokeWidth + 3, strokeWidth + 3, offset, height - strokeWidth - 3);
        //根据电池电量决定电池内矩形电量颜色
        if (mPower < 30) {
            paint.setColor(Color.parseColor("#F56A68"));
        }
        if (mPower >= 30 && mPower < 50) {
            paint.setColor(Color.parseColor("#C0F568"));
        }
        if (mPower >= 50) {
            paint.setColor(Color.parseColor("#C0F568"));
        }
